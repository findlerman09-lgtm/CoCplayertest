---
layout: character
permalink: /characters/clara-whitcombe/
name: Clara Whitcombe
occupation: Photographer
age: 34
residence: London
social_status: Middle class
portrait: /assets/images/clara-portrait.webp
hero_art: /assets/images/hero-collage.webp
tagline: "A meticulous commercial photographer who notices discrepancies other people would rather dismiss."
background: >-
  Clara earns her living behind a camera and beneath a dark cloth, making careful records for clients who want a room, a machine, an object, or a person fixed exactly as it appeared. She prefers subjects that cannot flatter her and has built a modest reputation for technically exact work. Her eye is exceptionally quick; the rest of her is not. Clara moves deliberately, dislikes being hurried, and routinely sees the troublesome detail before she has decided what to do about it.
history: >-
  Years ago, a well-paying client asked for a second plate after Clara's first image preserved an embarrassing defect the client wanted ignored. She refused to pretend the fault was not there. The argument cost her the commission and several weeks of income, but it also established the trait that now defines her professional life: if something is present, Clara believes it belongs in the record. She is not an investigator and does not think of herself as brave. She simply finds unresolved discrepancies almost impossible to leave alone.
roleplay_anchor: >-
  Accuracy is a form of respect. If the truth is inconvenient, that is not the camera's fault—and usually not Clara's either.
character_read: >-
  This page is the tone target for the whole project: evocative first impression, then practical dossier depth. Clara should feel like a person before she feels like a stat block.
human_detail: "She makes tea before beginning darkroom work and almost always discovers it cold hours later."
people:
  - name: Margaret Whitcombe
    relationship: Older sister
    text: >-
      Margaret is the person Clara trusts to tell her when precision has become stubbornness. Their affection is real, but Margaret worries that Clara's habit of correcting other people's versions of events will eventually put her in serious trouble.
  - name: Edwin Bell
    relationship: Newspaper acquaintance
    text: >-
      A practical sub-editor who occasionally sends small photographic commissions Clara's way. Bell values speed more than Clara does, but he also knows that when she says something in a plate is odd, it is worth looking twice.
  - name: Arthur Henshaw
    relationship: Former employer and professional rival
    text: >-
      Henshaw taught Clara much of the trade and believes she repaid him by becoming difficult, independent, and insufficiently grateful. He respects her work enough to resent it.
knowledge:
  - title: Plates remember without tact
    text: >-
      You are trained to separate flaws in exposure, chemistry, focus, and handling from details that were genuinely in front of the lens. You habitually examine edges, reflections, shadows, and small changes in repeated views.
  - title: The trade has a human network
    text: >-
      Printers, framers, stationers, photographic suppliers, newspaper staff, clients, and other photographers form the ordinary web of your professional life. You know who is careful, who is cheap, and who talks too much.
  - title: Observation is not interpretation
    text: >-
      You are very good at saying what is actually visible. You are slower to accept a dramatic explanation for it. This makes you useful when other people become certain too quickly.
sealed:
  - lock_id: clara-demo-letter
    title: Sealed correspondence
    subtitle: Demonstration item
    teaser: >-
      This is a proof-of-concept locked item. The Keeper can give the player a code word at the table, at which point the browser unlocks the text and remembers it on that device.
    salt: "8meVQdnUFRzv1mLg8gSmNw=="
    iv: "nAdTN52LkoZ6/U0Y"
    ciphertext: ""
updates:
  - title: How a personal clue will appear
    date: Demo only
    label: Non-canon example
    class: demo
    text: >-
      During play, a clue meant specifically for this character can be added here with the date it became available, a short description, and any image or transcript the player has actually received.
  - title: How a changed relationship will appear
    date: Demo only
    label: Non-canon example
    class: demo
    text: >-
      If an NPC's attitude, a debt, a promise, an injury, or a personal obligation changes in play, the dossier can record the new state here rather than forcing the player to remember it from a session weeks earlier.
characteristics:
  - { name: STR, value: 55, half: 27, fifth: 11 }
  - { name: CON, value: 55, half: 27, fifth: 11 }
  - { name: SIZ, value: 75, half: 37, fifth: 15 }
  - { name: DEX, value: 35, half: 17, fifth: 7 }
  - { name: APP, value: 40, half: 20, fifth: 8 }
  - { name: INT, value: 95, half: 47, fifth: 19 }
  - { name: POW, value: 45, half: 22, fifth: 9 }
  - { name: EDU, value: 90, half: 45, fifth: 18 }
vitals:
  - { name: Hit Points, value: 13 }
  - { name: Sanity, value: 45 }
  - { name: Luck, value: 75 }
  - { name: Magic Points, value: 9 }
  - { name: Build, value: 1 }
  - { name: Move, value: 7 }
skills:
  - { name: "Art/Craft (Photography)", value: 80, note: "Expert professional skill" }
  - { name: "Spot Hidden", value: 75, note: "Keen Vision also applies" }
  - { name: "Library Use", value: 70 }
  - { name: "Science (Chemistry)", value: 63 }
  - { name: "Psychology", value: 60 }
  - { name: "Stealth", value: 58 }
  - { name: "Listen", value: 55 }
  - { name: "First Aid", value: 50 }
  - { name: "Persuade", value: 50 }
  - { name: "Mechanical Repair", value: 45 }
  - { name: "Fighting (Brawl)", value: 40 }
  - { name: "History", value: 40 }
  - { name: "Ride", value: 40 }
  - { name: "Credit Rating", value: 35 }
  - { name: "Appraise", value: 30 }
  - { name: "Dodge", value: 30 }
  - { name: "Fast Talk", value: 30 }
  - { name: "Language (French)", value: 26 }
  - { name: "Navigate", value: 22 }
  - { name: "Language (Own: English)", value: 90 }
talent:
  name: Keen Vision
  text: "Gain a bonus die to Spot Hidden rolls. In this proof-of-concept it represents exceptional but wholly human visual acuity and trained observation."
combat:
  brawl: "40% (20/8), 1D3 + 1D4"
  dodge: "30% (15/6)"
  db: "+1D4"
  build: 1
  mov: 7
credit_rating: 35
wealth_note: "Comfortable enough to maintain professional equipment and a modest middle-class life, but not wealthy enough for money to solve problems by itself."
gear:
  - "Professional dry-plate camera with tripod and dark cloth"
  - "Glass plate holders and a small supply of prepared plates"
  - "Darkroom chemicals, trays, measuring glassware, and storage boxes"
  - "Magnifying glass and cleaning cloths"
  - "Notebook, pencils, writing case, and appointment cards"
  - "Weatherproof outer coat and a sturdy case for photographic equipment"
---
